<template>
  <div>
    <v-hover v-slot:default="{ hover }">
      <v-card :elevation="hover ? 4 : 2" class="mx-auto transition-swing" max-width="800">
        ...
        <v-container v-else class="text-center">
          ...
          <p class="body-1 mt-4 teal--text">
            {{ $t('about.sparklines.createOrder') }} {{ createOrderCount }}
            {{ $t('about.sparklines.orders') }}
          </p>

          <v-divider class="mx-4 mb-4" />

          <div class="title success--text">
            <v-icon color="success">mdi-briefcase-check-outline </v-icon>
          ...
          <p class="body-1 mt-4 success--text">
            {{ $t('about.sparklines.finishOrder') }} {{ finishedOrderCount }}
            {{ $t('about.sparklines.orders') }}
          </p>
          ...
          <div class="title orange--text" v-if="mvsolvers.length != 0">
            <v-icon color="orange">mdi-card-account-details-star-outline </v-icon>
            {{ $t('about.sparklines.contributor') }}
          </div>

          <v-row class="mt-4" v-if="mvsolvers.length != 0">
            <template v-for="solver in mvsolvers">
              <v-col cols="auto" v-bind:key="solver.user_id" class="mx-auto">
                <v-card elevation="1" max-width="300px" outlined>
                  <div class="ml-6 mr-6">
                    ...
                    <div class="text-subtitle-1 orange--text">
                      {{ $t('about.sparklines.finishOrderSimple') }}
                      {{ solver.finish_order_count }}
                      {{ $t('about.sparklines.orders') }}
                    </div>
                    <div class="pt-3 pb-6 text-body-2">
                      <span v-if="role === $GLOBAL.role.solver && solver.nickname"
                        >({{ solver.name }})
                      </span>
                      {{ solver.intro }}
                    </div>
...
export default {
  ...
  created () {
    ...
    this.axios
      .post('/api/about/queryMostValuableSolverMonthly')
      .then(Response => {
