<template>
  <div class="text-center">
    <v-bottom-sheet v-model="sheet" inset :persistent="submitLoading">
            ...
            <v-card-title>
              ...
              <v-tooltip left>
                ...
                <span v-show="!isModify">{{
                  $t('order.createOrder.solver.cancelTip')
                }}</span>
                <span v-show="isModify">{{
                  $t('order.modifyOrder.solver.cancelTip')
                }}</span>
              </v-tooltip>
              ...
              >
                <span v-if="!isModify">{{
                  $t('order.createOrder.solver.submit')
                }}</span>
                <span v-if="isModify">{{
                  $t('order.modifyOrder.solver.submit')
                }}</span>
              ...
              <v-row class="mt-6" justify="center">
                ...
                <v-col cols="3">
                  <v-text-field
                    ...
                    @blur="$v.name.$touch()"
                    :disabled="
                      modifiedOrder.order_status == $GLOBAL.status.finished ||
                ...
                <v-col cols="12">
                  <v-textarea
                    v-if="
                      status === this.$i18n.t('order.waitingStatus') ||
                        status === this.$i18n.t('order.receiptedStatus')
                    "
                    v-model="description"
    ...
    <v-dialog v-model="resetConfirmDialog" max-width="600px">
      <v-card>
        <v-card-title>
          <span class="headline">{{
            $t('order.createOrder.solver.reset')
          }}</span>
        </v-card-title>
        <v-card-text>
          <v-container>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn @click="resetConfirmDialog = false" class="subtitle-2">{{
            $t('order.createOrder.solver.cancel')
          }}</v-btn>
          <v-btn color="primary" @click="reset" class="subtitle-2">{{
            $t('order.createOrder.solver.resetConfirm')
          }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  computed: {
    nameErrors () {
      ...
      !this.$v.name.maxLength &&
        errors.push(this.$i18n.t('order.createOrder.solver.nameMaxLengthErr'))
      return errors
    },
    genderErrors () {
      const errors = []
      ...
      !this.$v.gender.required &&
        errors.push(this.$i18n.t('order.createOrder.solver.genderRequiredErr'))
      return errors
    },
    campusErrors () {
      const errors = []
      ...
      !this.$v.campus.required &&
        errors.push(this.$i18n.t('order.createOrder.solver.campusRequiredErr'))
      return errors
    },
    dormitoryErrors () {
      const errors = []
      ...
      !this.$v.dormitory.required &&
        errors.push(
          this.$i18n.t('order.createOrder.solver.dormitoryRequiredErr')
        )
      return errors
    },
    telephoneErrors () {
      ...
      !this.$v.telephone.numeric &&
        errors.push(
          this.$i18n.t('order.createOrder.solver.telephoneNumericErr')
        )
      !this.$v.telephone.maxLength &&
        errors.push(
          this.$i18n.t('order.createOrder.solver.telephoneMaxLengthErr')
        )
      return errors
    },
    statusErrors () {
      ...
      !this.$v.status.required &&
        errors.push(this.$i18n.t('order.createOrder.solver.statusRequiredErr'))
      return errors
    },
    nameLabel () {
    ...
    statusItems () {
      return [
        this.$i18n.t('order.waitingStatus'),
        this.$i18n.t('order.receiptedStatus'),
        this.$i18n.t('order.recordedStatus')
      ]
    },
    statusLabel () {
  ...
  methods: {
      ...
      if (!this.isModify) {
        ...
        if (
          ...
          if (this.$DevMode) {
            this.$Bus.$emit(
              'setSnackbar',
              this.$i18n.t('order.createOrder.solver.createSucceed')
            )
            this.sheet = false
            return
          }
          ...
          if (this.status === this.$i18n.t('order.waitingStatus')) {
            ...
            this.$Bus.$emit(
              'setSnackbar',
              this.$i18n.t('order.createOrder.solver.createFailed')
            )
            location.reload()
          }

          this.axios
            .post('/api/order/createOrderSolver', {
              user_name: this.name,
        ...
        if (
          ...
          this.axios
            .post('/api/order/modifyOrderSolver', {
            ...
            .then(Response => {
              if (Response.data === false) {
                ...
                this.$Bus.$emit(
                  'setSnackbar',
                  this.$i18n.t('order.modifyOrder.solver.modifyFailed')
                )
                location.reload()
              } else {
                // 订单信息修改成功
                this.$Bus.$emit(
                  'setSnackbar',
                  this.$i18n.t('order.modifyOrder.solver.modifySucceed')
                )
    ...
    setOrderData (order) {
      ...
      if (
        orderUserGender == '男' ||
        orderUserGender == this.$GLOBAL.gender.male
      ) {
        this.gender = this.genderItems[0]
      } else if (
        orderUserGender == '女' ||
        orderUserGender == this.$GLOBAL.gender.female
      ) {
      ...
      if (
        orderUserCampus == '清水河校区' ||
        orderUserCampus == '清水河' ||
        orderUserCampus == this.$GLOBAL.campus.qingshuihe
      ) {
        this.campus = this.campusItems[0]
      } else if (
        orderUserCampus == '沙河校区' ||
        orderUserCampus == '沙河' ||
        orderUserCampus == this.$GLOBAL.campus.shahe
      ) {
      ...
      if (
        ...
        this.$Bus.$emit(
          'setSnackbar',
          this.$i18n.t('order.modifyOrder.solver.getOrderStatusFailed')
        )
        location.reload()
      }
      // 设置用户描述
  ...
  mounted () {
    this.$Bus.$on('openCreateOrderSolverSheet', msg => {
      // 创建订单模式
      if (this.isModify) {
        // 从修改订单模式进入创建订单模式，从 localStorage 读取存储的表单数据
    ...
    this.$Bus.$on('openModifyOrderSolverSheet', msg => {
      // 修改订单模式
      if (!this.isModify) {
        // 从创建订单模式进入修改订单模式，保存原本表单数据到 localStorage
