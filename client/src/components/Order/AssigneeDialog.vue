<template>
  <div>
    ...
    >
      <v-card>
        <v-card-title>{{ $t('order.assigneeDialog.confirmTitle') }}</v-card-title>
...
export default {
  ...
  methods: {
    ...
    addAssignee () { // 添加协作人
      ...
      if (stdId == '') {
        // 搜索学号不能为空
        this.searchResultText = this.$i18n.t('order.assigneeDialog.searchNullError')
        return false
      } else if (stdId == this.order.solver_std_id) {
        // 协作人不能为处理者自己
        this.searchResultText = this.$i18n.t('order.assigneeDialog.addYouAsAssigneeFailed')
        return false
      } else {
        // 检验协作人是否已存在本订单
        for (let i = 0; i < this.assignee.length; i++) {
          if (this.assignee[i].solver_std_id == stdId) {
            this.searchResultText = this.$i18n.t('order.assigneeDialog.addAssigneeExisted') + this.assignee[i].solver_name
            return false
          }
        }
      }
      ...
      this.axios.post('/api/user/getSolverInfoByStdId', {
        ...
        if (Response.data) { // 成功获取到处理者信息
          ...
          // 未获取到处理者信息或其他错误
          this.searchResultText = this.$i18n.t('order.assigneeDialog.searchAssigneeFailed')
          this.addAssigneeLoading = false
          return false
        }
      })
